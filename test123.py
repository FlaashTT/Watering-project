...
text_message1 = 'Valor de Humidade: {}'.format(soil_moisture)
text_message3 = 'Valor de Profundidade: {}'.format(rounded)
...
try:
    ...
    while True:
        now = datetime.datetime.now()
        if now.hour == 20 and now.minute == 37:
            if soil_moisture > 170 and rain_sensor_state == 1: 
                print("Sem chuva - ligando a bomba")
                GPIO.output(VALVE_PIN, GPIO.HIGH)
                GPIO.output(RELAY_PIN, GPIO.HIGH)
                time.sleep(4)
                
                # Medir o fluxo de água enquanto a bomba estiver ligada
                while soil_moisture > 170:
                    tempo_atual = time.time()
                    delta_tempo = tempo_atual - tempo_anterior
                    tempo_anterior = tempo_atual
                    fluxo = pulsos / (delta_tempo * 60) * 7.5
                    
                    text_message2 = 'Valor de Fluxo: {}'.format(fluxo)
                    SendShortMessage(phone_number, text_message2)
                ...
                SendShortMessage(phone_number, text_message3)
                time.sleep(10)
            elif soil_moisture < 160 or rain_sensor_state == 0:
